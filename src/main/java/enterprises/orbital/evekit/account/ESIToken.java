...
public class ESIToken {
  ...
  public static ESIToken createKey(final EveKitUserAccount userAccount, final String scopes,
  ...
  public static ESIToken getKeyByID(final long kid) {
    try {
      return EveKitUserAccountProvider.getFactory().runTransaction(() -> {
          TypedQuery<ESIToken> getter = EveKitUserAccountProvider.getFactory().getEntityManager()
  ...
  public static List<ESIToken> getAllKeys(final EveKitUserAccount userAccount) {
    try {
      return EveKitUserAccountProvider.getFactory().runTransaction(() -> {
          TypedQuery<ESIToken> getter = EveKitUserAccountProvider.getFactory().getEntityManager()
  ...
  public static boolean deleteKey(final EveKitUserAccount userAccount, final long kid) {
    try {
      EveKitUserAccountProvider.getFactory().runTransaction(() -> {
          ESIToken key = getKeyByID(kid);
          if (key != null && key.getUserAccount().equals(userAccount))
            EveKitUserAccountProvider.getFactory().getEntityManager().remove(key);
        });
      return true;
    } catch (Exception e) {
      log.log(Level.SEVERE, "query error", e);
  ...
  public static ESIToken update(final ESIToken key) {
    try {
      return EveKitUserAccountProvider.getFactory().runTransaction(() -> {
          return EveKitUserAccountProvider.getFactory().getEntityManager().merge(key);
        });
    } catch (Exception e) {
      log.log(Level.SEVERE, "query error", e);
      return null;
